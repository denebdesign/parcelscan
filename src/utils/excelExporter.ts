...
export const COURIER_CONFIGS: Record<CourierType, CourierConfig> = {
  cj: {
    ...
    badgeText: 'CJ대한통운 (주소 합침)',
    description: 'CJ대한통운 CNPlus/LoIS 전용 (기본+상세 주소 합침 1개 열)',
    headers: [
      '받는분성명',
      '받는분전화번호',
      '받는분기타연락처',
  ...
  lotte: {
    ...
    badgeText: '롯데택배 (주소 합침)',
    description: '롯데택배 ALPS 접수용 표준 (기본+상세 주소 합침 1개 열)',
    headers: [
      '수하인명',
      '수하인전화번호',
      '수하인휴대폰번호',
  ...
  hanjin: {
    ...
    badgeText: '한진택배 (주소 합침)',
    description: '한진택배 포털 일괄 접수용 (기본+상세 주소 합침 1개 열)',
    headers: [
      '받는분성명',
      '받는분전화번호1',
      '받는분전화번호2',
  ...
  post: {
    ...
    badgeText: '우체국택배 (기본·상세 분리)',
    description: '인터넷우체국 e-Post 다량배송 전용 (기본주소/상세주소 2개 열 분리)',
    headers: [
      '받는분',
      '받는분전화번호',
      '받는분핸드폰',
  ...
  logen: {
    ...
    badgeText: '로젠택배 (주소 합침)',
    description: '로젠택배 iLogen 연동 엑셀 양식 (기본+상세 주소 합침 1개 열)',
    headers: [
      '수하인명',
      '수하인전화1',
      '수하인전화2',
  ...
  standard: {
    ...
    badgeText: '범용 통합 (주소 합침)',
    description: '받는분주소(기본+상세)가 하나로 합쳐진 범용 통합 엑셀 양식',
    headers: [
      '순번',
      '받는분',
      '연락처',
...
export function formatDataForCourier(
  ...
  const rows = items.map((item, index) => {
    ...
    const sPhone = item.senderPhone || sender.phone || '010-0000-0000';
    const sAddress = item.senderAddress || `${sender.address} ${sender.detailAddress}`.trim() || '발송지 주소';
    const fullAddress = [item.address, item.detailAddress].filter(Boolean).join(' ').trim();
